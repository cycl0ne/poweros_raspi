...
static int mmc_bounce_need_bounce(struct mmc_data *orig)
{
	...
	if (orig->flags & MMC_DATA_READ)
		addr = (UINT32)orig->dest;
	else
		addr = (UINT32)orig->src;
	...
	len = (UINT32)(orig->blocksize * orig->blocks);
...
static UINT32 mmc_erase_t(struct mmc *mmc, UINT32 start, lbaint_t blkcnt)
{
	struct mmc_cmd cmd;
	UINT32 end;
...
static UINT32
mmc_berase(INT32 dev_num, UINT32 start, lbaint_t blkcnt)
{
	int err = 0;
...
static UINT32
mmc_write_blocks(struct mmc *mmc, UINT32 start, lbaint_t blkcnt, const void*src)
{
	struct mmc_cmd cmd;
...
static UINT32
mmc_bwrite(int dev_num, UINT32 start, lbaint_t blkcnt, const void*src)
{
...
int mmc_read_blocks(struct mmc *mmc, void *dst, UINT32 start, lbaint_t blkcnt)
{
	struct mmc_cmd cmd;
	struct mmc_data data;
...
static UINT32 mmc_bread(INT32 dev_num, UINT32 start, lbaint_t blkcnt, void *dst)
{
	lbaint_t cur, blocks_todo = blkcnt;
...
int sd_switch(struct mmc *mmc, int mode, int group, UINT8 value, UINT8 *resp)
{
	struct mmc_cmd cmd;
	struct mmc_data data;
...
int sd_change_freq(struct mmc *mmc)
{
	...
	ALLOC_CACHE_ALIGN_BUFFER(UINT32, switch_status, 16);
	struct mmc_data data;
	int timeout;
