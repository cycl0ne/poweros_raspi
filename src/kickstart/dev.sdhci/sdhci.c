...
static void sdhci_reset(struct sdhci_host *host, UINT8 mask)
{
...
static void sdhci_transfer_pio(struct sdhci_host *host, struct mmc_data *data)
{
	...
	for (i = 0; i < data->blocksize; i += 4) {
		...
		if (data->flags == MMC_DATA_READ)
			*(UINT32 *)offs = sdhci_readl(host, SDHCI_BUFFER);
		else
			sdhci_writel(host, *(UINT32 *)offs, SDHCI_BUFFER);
	}
}
...
int sdhci_init(struct mmc *mmc)
{
	...
	if ((host->quirks & SDHCI_QUIRK_32BIT_DMA_ADDR) && !aligned_buffer) {
		aligned_buffer = malloc(512*1024);//memalign(8, 512*1024);
		if (!aligned_buffer) {
...
int add_sdhci(struct sdhci_host *host, UINT32 max_clk, UINT32 min_clk)
{
	struct mmc *mmc;
	unsigned int caps;
