...
#define USB_PRINTF(fmt, args...)	//printf(fmt, ##args)//debug_cond(USB_DEBUG, fmt, ##args)
#define USB_HUB_PRINTF(fmt, args...)	//printf(fmt, ##args)//debug_cond(USB_HUB_DEBUG, fmt, ##args)
...
STRPTR print_request(unsigned char request)
{
	switch(request)
	{
			...
			void *data, unsigned short size, int timeout)
{
	...
	USB_PRINTF("usb_control_msg: request: %s, requesttype: 0x%X, " \
		   "value 0x%X index 0x%X length 0x%X\n",
		   print_request(request), requesttype, value, index, size);
 ...
 */
static void __attribute__ ((noinline)) usb_set_maxpacket_ep(struct usb_device *dev, int if_idx, int ep_idx)
{
	int b;
...
static void usb_scan_devices(void)
{
	...
	if (usb_new_device(dev))
		printf2("No USB Device found\n");
	else
		printf2("%d USB Device(s) found\n", dev_index);
