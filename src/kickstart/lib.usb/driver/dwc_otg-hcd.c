		       ...
		       int len,struct devrequest *setup)
{
	...
	if (buffer) {
		...
		if ((UINT32)buffer & 0x7)
		{
			printf("buffer: [%x] is not aligned", buffer);
		}
	...
	if ((UINT32)status_buffer & 0x7)
	{
		printf("status_buffer: [%x] is not aligned", status_buffer);
	}	
